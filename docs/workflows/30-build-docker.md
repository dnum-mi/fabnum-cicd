...
| TAG_SHORT_SHA       | boolean | Taguer avec le SHA court du commit                                                                                                                                                                                                                  | Non    | `true`             |
...
- `TAG_SHORT_SHA` (défaut `true`) ajoute un tag portant le SHA court du commit, pour la traçabilité. À désactiver pour un appelant dont les branches sont rebasées ou poussées en force : un commit qui ne change pas le contenu de l'image réutilise le digest précédent, et le nouveau SHA s'ajoute alors comme un tag de plus sur cette version toujours vivante au lieu de marquer quoi que ce soit de nouveau - ces tags s'accumulent tant que la branche existe, puisque seule une version entière peut être supprimée, jamais un tag isolé.
